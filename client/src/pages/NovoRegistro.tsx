...
import { useAppContext, PRODUCTS, OPERATIONS } from "@/lib/store";
...
export default function NovoRegistro() {
  ...
  // Use simple get instead of full URLSearchParams to avoid TS errors
  const [openCamera, setOpenCamera] = useState(false);
  
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    setOpenCamera(params.get('camera') === 'true');
  }, []);
  ...
  useEffect(() => {
    if (openCamera && step === 1 && !photo) {
      // Trigger camera click after render if requested
      const timer = setTimeout(() => {
         fileInputRef.current?.click();
      }, 500);
      return () => clearTimeout(timer);
    }
  }, [openCamera, step, photo]);
  ...
  const handleCapture = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      // In a real app, upload to storage. For mockup, create object URL
  ...
  const handleSave = () => {
    if (!product || !quantity || !operation) {
      toast({
        ...
        description: "Preencha Produto, Quantidade e Operação.",
        variant: "destructive"
      });
      return;
    }
    ...
    <div className="flex flex-col h-full bg-muted/30">
      ...
      <div className="p-4 flex-1 flex flex-col max-w-md mx-auto w-full">
        ...
        {step === 2 && (
          <div className="space-y-6 flex-1">
            ...
            <div className="space-y-4 bg-card p-5 rounded-2xl border shadow-sm">
              ...
              <div className="space-y-2">
                <Label className="text-base text-muted-foreground">Nota Fiscal (NF) / Lote</Label>
                <div className="flex gap-2">
                  <Input 
                    type="text" 
            ...
            <div className="flex gap-3 pt-4">
              ...
              <button 
                onClick={() => {
                  if (!quantity) {
                    toast({title: "Atenção", description: "Informe a quantidade", variant: "destructive"});
                    return;
                  }
                  setStep(3);
                }}
                className="flex-[2] py-4 rounded-xl bg-primary text-primary-foreground font-bold text-lg active:scale-95 transition-transform"
              >
                Próximo
              </button>
            </div>
        ...
        {step === 3 && (
          <div className="space-y-6 flex-1">
            <div>
              <h2 className="text-2xl font-bold mb-2">3. Tipo de Operação</h2>
              <p className="text-muted-foreground text-sm">Selecione o que está sendo feito</p>
            </div>
            ...
            <div className="grid gap-3">
              {OPERATIONS.map(op => (
                <button
                  ...
                  className={`p-5 rounded-2xl text-left font-bold text-lg border-2 transition-all flex justify-between items-center
                    ${operation === op 
                      ? 'border-primary bg-primary/5 text-primary' 
                      : 'border-border bg-card hover:bg-muted'}`}
                >
                  {op}
                  {operation === op && <CheckCircle2 className="text-primary" />}
                </button>
              ))}
            </div>
            ...
            {/* Summary Box */}
            {operation && (
              <div className="bg-primary/5 p-4 rounded-xl border border-primary/20 space-y-2 mt-4">
                <p className="text-sm"><span className="font-bold text-muted-foreground">Produto:</span> {product}</p>
                <p className="text-sm"><span className="font-bold text-muted-foreground">Qtd:</span> {quantity} kg</p>
                {nf && <p className="text-sm"><span className="font-bold text-muted-foreground">NF:</span> {nf}</p>}
                <p className="text-sm"><span className="font-bold text-muted-foreground">Operação:</span> {operation}</p>
              </div>
            )}
